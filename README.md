...
校内测试环境下可完整实现全部功能：静止模式钢球定位误差≤±5mm，循迹模式动态保持误差≤±1cm；
省赛现场受场地光线差异、运输导致机械结构偏移、电池电压下降等综合因素影响，未完成相关测试项，代码与文档已完整开源。

## 个人贡献
本项目为三人团队参赛作品，本人独立负责全部STM32端软件开发与系统联调，包括：
- **全外设驱动开发**：USART1 DMA循环接收、TIM3舵机PWM、TIM4四路电机PWM、软件I2C OLED、红外循迹
- **控制算法**：两套 PID 参数（静止/循迹分开调）、目标限速跟踪、分模式状态机
- **通信与鲁棒性**：OpenMV ASCII 协议逐字节解析、按帧类型分级处理、停止线防误触
- **整机功能调试与现场问题复盘**

OpenMV视觉识别算法与机械结构设计由团队队友完成。
